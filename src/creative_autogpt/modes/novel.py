        ...
        if goal_idea:
            ...
            for keyword, description in dynasty_keywords.items():
                if keyword in goal_idea:
                    worldview_constraints.append(f"⛔ 时代锁定：{description}，严禁出现其他朝代人物或事件")
                    worldview_constraints.append(f"⛔ 禁止出现其他时代的人物（如{keyword}不能出现秦始皇、李世民、朱元璋等）")

            # 现代都市关键词
            modern_keywords = ["现代", "都市", "2024", "21世纪", "当代"]
            if any(kw in goal_idea for kw in modern_keywords):
                worldview_constraints.append("⛔ 时代锁定：现代都市，严禁出现古代人物或穿越元素")

            # 穿越/系统设定
            if "穿越" in goal_idea or "系统" in goal_idea:
                if "穿越" in goal_idea:
                    worldview_constraints.append("✓ 允许穿越设定，但穿越前后的时代必须明确区分")
                if "系统" in goal_idea:
                    worldview_constraints.append("✓ 允许系统设定，但系统不能凭空改变世界观规则")
