...
from creative_autogpt.core.modular_structure import (
    ModularStructurePlanner,
    NovelStructure,
    StoryModule,
    ActType,
    ChapterType,
...
📚 修仙小说通用写作标准（融合各类修仙经典）：
✅ 必须做到：
- **升级体系清晰**：炼气→筑基→金丹→元婴→化神→渡劫→大乘，每个境界有明确划分
- **功法体系完整**：功法等级（天地玄黄）、修炼条件、效果描述、限制代价
- **世界层层递进**：新手村→宗门→州城→中州→上界→仙界，地图随实力解锁
- **战斗体系合理**：越级挑战有代价，同阶战斗看技巧，高阶压制需智斗
- **资源争夺真实**：天材地宝、功法秘籍、洞府遗迹引发合理冲突
- **心性修为并重**：境界提升需心境匹配，避免"德不配位"
...
💡 核心要素：
- **金手指合理**：系统/重生/宝物要有使用限制和代价
- **打脸适度**：爽点要自然，不能强行降智反派
- **节奏把控**：张弛有度，战斗、修炼、日常交替
- **人物成长**：主角性格随经历成熟，配角有独立故事线
""",
        "都市修仙": """
📚 都市修仙小说写作标准（参考《我的徒弟都是大反派》《最强反套路系统》等）：
✅ 必须做到：
- **古今结合**：修仙体系与现代都市生活完美融合
- **日常修仙化**：修炼融入日常（地铁上打坐、外卖送丹药、扫码支付灵石）
- **反差萌点**：仙人也要面对生活琐事（房租、工作、人际关系）
- **节奏明快**：对话生动，情节紧凑，不拖泥带水
- **爽点清晰**：打脸、升级、装逼要自然不尴尬
- **现代冲突**：用修仙手段解决现代问题（治病、破案、商业竞争）
...
💡 特色写法：
- **身份反差**：表面普通学生/上班族，暗中是修仙大佬
- **生活修仙化**：用电瓶车代替飞剑，用外卖箱装丹药，用微信群交流修仙心得
- **现代梗融入**：适当使用网络流行语和热点梗
- **情感真实**：修仙者也要面对亲情、爱情、友情的现代困境
""",
        "历史修仙": """
📚 历史修仙小说写作标准（参考《长生界》《大周仙吏》等）：
✅ 必须做到：
- **历史背景扎实**：融入真实朝代（唐宋明清），考据基本史实
- **修仙与历史交织**：历史人物、事件与修仙体系巧妙结合
- **朝代特色鲜明**：不同朝代有不同的修仙文化（唐尚道、宋崇文、明清神秘）
- **长生视角**：主角跨越朝代，见证历史变迁，积累底蕴
- **朝堂与江湖**：官场斗争与修仙界的权力博弈相结合
...
💡 特色写法：
- **见证历史**：主角亲历重大历史事件，从修仙者视角解读
- **历史人物修仙化**：李白是剑仙、苏轼是文圣、秦始皇求长生
- **朝代修仙差异**：唐朝剑修盛行、宋朝丹道发达、明朝符箓流行
- **时间跨度**：主角活几百年，见证朝代更迭，积累实力
""",
        "传统修仙": """
📚 古典修仙小说写作标准（融合儒道哲学智慧）：
✅ 必须做到：
- **道法自然**：修炼顺应天道规律，不可逆天而行
- **内圣外王**：修为提升伴随心性修养，德位相配
- **阴阳调和**：力量与智慧的平衡，刚柔并济
- **天人合一**：人与自然、宇宙的和谐统一
- **知行合一**：理论领悟与实际行动的统一
...
- **知足常乐**：知足不辱，知止不殆
- **返璞归真**：大道至简，去伪存真
- **阴阳五行**：功法体系遵循阴阳相生、五行相克原理

📖 儒家思想融入：
- **仁义礼智信**：人物核心价值观（仁者爱人、义者宜也）
- **修身齐家治国平天下**：修炼者从自我完善到造福苍生
- **君子坦荡荡**：主角胸怀宽广，小人长戚戚
- **中庸之道**：不偏不倚，恰到好处
- **格物致知**：通过观察万物领悟天道
...
💡 哲学元素应用场景：
- **世界观构建**：天道规则体现道家"道法自然"，社会伦理体现儒家"仁义礼智"
- **门派设定**：道家门派重自然无为，儒家门派重德行修养，法家门派重秩序规则
- **人物设计**：主角持"中庸"之道，反派走"极端"之路，形成价值观冲突
- **突破场景**：不仅是力量暴涨，更是"悟道"时刻（如"一朝悟道，立地成仙"）
- **对话设计**：长老教导弟子时引用《道德经》《论语》经典
- **功法原理**：功法描述融入阴阳五行、太极八卦、天人合一等概念

📜 经典引用示例：
- "道可道，非常道" —— 描述天道之玄妙
- "上善若水，水善利万物而不争" —— 描述水属性功法最高境界
- "仁者爱人" —— 主角的核心信念
- "天行健，君子以自强不息" —— 激励主角突破困境
- "知其雄，守其雌" —— 以柔克刚的战术思想
...
"""
    ...
    def __init__(
        ...
        # 🔥 模块化三幕结构规划器（用于长篇网络小说）
        self.modular_structure_planner = ModularStructurePlanner(config=config)
        self.novel_structure: Optional[NovelStructure] = None  # 将在plan阶段初始化
    ...
    async def run(
        ...
        try:
            ...
            tasks = await self.planner.plan(
                ...
                modular_structure=self.novel_structure,  # 🔥 传递模块化三幕结构
            )

            # 🔥 初始化模块化三幕结构规划
            if chapter_count and chapter_count > 0:
                logger.info(f"🎭 Initializing modular three-act structure for {chapter_count} chapters")
                self.novel_structure = self.modular_structure_planner.plan_structure(
                    title=goal.get('title', 'Untitled'),
                    total_chapters=chapter_count,
                    genre=goal.get('genre', 'cultivation'),
                    outline='',  # 将在大纲生成后更新
                )
                logger.info(f"✅ Modular structure planned: {len(self.novel_structure.modules)} modules")
                # 打印结构概览
                for module in self.novel_structure.modules:
                    logger.info(f"  Module {module.module_number}: {module.title} "
                              f"(Chapters {module.start_chapter}-{module.end_chapter}, "
                              f"{module.get_chapter_count()} chapters, "
                              f"{module.power_level_start}→{module.power_level_end})")
...
对于{chapter_count}章的长篇小说，采用**无限重复的模块化三幕结构**：

**核心原则**：
- 将{chapter_count}章分成多个**故事模块**（每个模块约100章）
- 每个模块内部使用完整的三幕结构（20%-60%-20%）
- 模块之间通过"换地图/升级"自然衔接
- 形成"新手村→宗门→州城→上界→仙界"的阶梯式升级

**模块划分建议**（根据章节数自动调整）：

每个模块约100章，内部使用20%-60%-20%的三幕结构：

| 模块 | 章节范围 | 世界层级 | 修为境界 | 模块标题 |
|-----|---------|---------|---------|---------|
| 1 | 第1-100章 | 凡俗界 | 炼气期 | 初入修仙路 |
| 2 | 第101-200章 | 宗门外围 | 筑基期 | 宗门风云 |
| 3 | 第201-300章 | 宗门核心 | 金丹期 | 真传弟子 |
| 4 | 第301-400章 | 州城 | 元婴期 | 州城争霸 |
| 5 | 第401-500章 | 中州 | 化神期 | 中州风云 |
| ... | ... | ... | ... | ... |

**每个模块的三幕结构**：
- **第一幕（20%）**：进入新世界/新地图，建立新规则，引发事件
- **第二幕（60%）**：升级修炼，积累矛盾，中点转折，对抗反派
- **第三幕（20%）**：最终决战，解决冲突，离开当前世界，铺垫下一模块
...
**模块1：初入修仙路（第1-100章）**
- 第一幕（第1-20章）：进入新手村，触发金手指
- 第二幕（第21-80章）：修炼升级，打脸反派，建立势力
- 第三幕（第81-100章）：离开新手村，前往宗门
...
**模块2：宗门风云（第101-200章）**
- 第一幕（第101-120章）：入门测试，成为外门弟子
- 第二幕（第121-180章）：外门竞争，晋升内门，中点转折
- 第三幕（第181-200章）：宗门大比，获得真传，离开外门

...以此类推，规划全部{chapter_count}章

⚠️ **重要**：
1. 每个模块必须有完整的三幕结构
2. 模块之间通过"换地图"自然衔接
3. 主角修为随模块提升（炼气→筑基→金丹...）
4. 每个模块都有独立的反派和冲突
5. 所有{chapter_count}章都要规划，不能省略
...
"""
            ...
            if isinstance(chapter_index, int) and self.novel_structure:
                guidelines = self.modular_structure_planner.get_chapter_guidelines(chapter_index, self.novel_structure)
                if guidelines:
                    module = guidelines.get("module", "未知")
                    module_title = guidelines.get("module_title", "")
                    act = guidelines.get("act", "未知")
                    chapter_type = guidelines.get("chapter_type", "未知")
                    progress = guidelines.get("progress_in_module", 0) * 100
                    world_level = guidelines.get("world_level", "")
                    power_level = guidelines.get("power_level", "")
                    beats = guidelines.get("required_beats", [])
                    
                    modular_structure_info = f"""
### 🎭 模块化三幕结构指导

**当前位置**：第 {chapter_index} 章（全书共 {chapter_count} 章）

**所属模块**：第 {module} 模块 - {module_title}
  - 本模块进度：{progress:.1f}%
  - 世界层级：{world_level}
  - 修为境界：{power_level}
...
"""
                    for i, beat in enumerate(beats, 1):
                        modular_structure_info += f"    {i}. {beat}\n"
                    
                    # 添加三幕特定的写作指导
                    if "第一幕" in act:
                        modular_structure_info += """
**第一幕写作要求**（Setup - 建立）：
- 如果是模块第一章：引入新地图/新世界，建立新环境
- 展示当前世界层级的规则和特色
- 建立本模块的目标和冲突
- 引发事件：打破现状，迫使主角行动
- 铺垫本模块的反派势力
...
"""
                            ...
                            modular_structure_info += """
**第三幕后期写作要求**（Climax & Resolution - 高潮与解决）：
- 最终对决：与模块反派的决战
- 展现主角成长成果
- 解决本模块的核心冲突
- 收尾本模块，铺垫下一模块
...
2. **模块化三幕结构要求**：
   - 将{chapter_count}章分成多个模块（每模块约100章）
   - 每个模块内部使用完整的三幕结构（20%-60%-20%）
   - 模块1：第1章到第100章（新手村，炼气期）
   - 模块2：第101章到第200章（宗门外围，筑基期）
   - 模块3：第201章到第300章（宗门核心，金丹期）
   - ...以此类推
   - 每个模块都必须有完整的三幕（第一幕/第二幕/第三幕）

3. **章节规划必须覆盖全部{chapter_count}章**，按模块组织，不要用"..."省略
...
5. **不要输出标题或额外说明**，直接从"一、故事核心"开始输出

⚠️ 如果内容太长被截断，请优先确保：
- ✅ 所有模块都有完整的三幕结构
- ✅ 每个模块至少规划前20章（第一幕+第二幕开始）
- ✅ 人物列表完整
- ✅ 模块之间的过渡和衔接清晰
""")
        else:
