...
"""
    ...
    def _get_predecessor_contents(
        ...
        # 流程：创意脑暴 → 大纲 → 世界观规则 → 人物设计 → 主题确认/风格元素 → 市场定位 → 事件 → 场景物品冲突 → 伏笔列表 → 章节创作
        ...
        task_dependencies = {
            ...
            "人物设计": ["大纲", "世界观规则"],  # 人物在世界规则内完成大纲

            # Phase 4: 主题与风格（从故事中提炼）
            "主题确认": ["大纲", "世界观规则", "人物设计"],  # 主题从人物选择中涌现
            "风格元素": ["大纲", "世界观规则", "人物设计"],  # 风格服务于故事
            "市场定位": ["大纲", "人物设计", "风格元素"],  # 综合所有元素

            # Phase 5: 细节填充（为大纲添加血肉）
            "事件": ["大纲", "世界观规则", "人物设计", "市场定位"],
            "场景物品冲突": ["大纲", "世界观规则", "人物设计", "事件"],
            "伏笔列表": ["大纲", "人物设计", "事件", "场景物品冲突"],

            # Phase 6: 章节创作 - 🔴 必须包含所有基础设定 + 风格元素！
            # 基础设定 = 大纲 + 世界观规则 + 人物设计 + 事件 + 场景物品冲突 + 伏笔列表
            # 上一章内容通过 _get_previous_chapters() 单独获取
            "章节大纲": ["大纲", "世界观规则", "人物设计", "风格元素", "事件", "场景物品冲突", "伏笔列表"],
            "场景生成": ["大纲", "世界观规则", "人物设计", "风格元素", "事件", "场景物品冲突", "伏笔列表"],
            "章节内容": ["大纲", "世界观规则", "人物设计", "风格元素", "事件", "场景物品冲突", "伏笔列表"],
            "章节润色": ["大纲", "世界观规则", "人物设计", "风格元素", "事件", "场景物品冲突", "伏笔列表"],
        }
...
""")
    ...
    def _build_foundation_reference(
        ...
        foundation_tasks = ["大纲", "世界观规则", "人物设计", "事件", "场景物品冲突", "伏笔列表"]
...
"""
        ...
        # 🔥 混合方案：正确分类任务类型
        strategy_tasks = ["创意脑暴"]  # 🔴 策略规划任务 - 不直接写小说内容
